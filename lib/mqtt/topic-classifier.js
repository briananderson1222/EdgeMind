...
const TOPIC_PATTERNS = [
  ...
  // Sparkplug B: Node data (NDATA)
  {
    ...
    extract: (m) => {
      // If group_id is a namespace (jpi, opto22), use edge_node as enterprise
      const namespaces = ['jpi', 'opto22', 'infrastructure', 'phoenixcontact'];
      const enterprise = namespaces.includes(m[1]) ? m[2] : m[1];
      return {
...
function classifyTopic(topic) {
  ...
  // Skip known namespace prefixes that aren't enterprises
  const namespaces = ['abelara', 'spBv1.0', 'opto22', 'roeslein', 'aveva', 'maintainx', 'eukodyne'];
  const nonEnterprises = ['infrastructure', 'null', 'unknown'];
  let offset = 0;
  if (namespaces.includes(parts[0])) {
    offset = 1;
  }
  
  // Check if resolved enterprise is actually a non-enterprise
  let enterprise = parts[offset] || 'unknown';
  if (nonEnterprises.includes(enterprise)) {
    enterprise = null;
  }
  
  return {
    pattern: 'unknown',
    type: enterprise === null ? 'infrastructure' : 'unknown',
    tags: {
      enterprise: enterprise,
      site: parts[offset + 1] || 'unknown',
      area: parts[offset + 2] || 'unknown',
      machine: parts[offset + 3] || 'unknown',
      device: parts[offset + 4] || 'unknown'
    },
    hasFanOut: false
  };
}
...
function getInfluxTags(topic) {
  ...
  // Standardize OEE measurement names
  if (type === 'oee' && tags.metric) {
    tags.field = `oee_${tags.metric.toLowerCase()}`;
  }
...
function extractPayloadValue(payload) {
  if (payload.startsWith('{')) {
    try {
      ...
      // Recursively find numeric value
      const findValue = (obj, depth = 0) => {
        ...
        // Check common value field names (case-insensitive)
        for (const key of ['Value', 'value', 'val', 'v']) {
          if (key in obj) {
            // Explicitly null means no value - return special marker
            if (obj[key] === null) return { __null: true };
            const found = findValue(obj[key], depth + 1);
            if (found !== null && typeof found !== 'object') return found;
            if (found && found.__null) return found;
          }
        }
        // Check Name for state values - but NOT if this looks like a KPI object
        if (obj.Name && !obj.Formula && !obj.UnitsOfMeasure) return obj.Name;
        return null;
      };
      
      let value = json.value !== undefined ? findValue(json.value) : findValue(json);
      // Handle null marker - return null to indicate no value to write
      if (value && value.__null) return { value: null, quality };
