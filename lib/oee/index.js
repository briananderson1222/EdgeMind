 ...
 */
const OEE_TIERS = {
  0: 'data-type-tag',        // NEW: Query by data_type tag (most reliable for new data)
...
function analyzeEnterpriseOEE(enterprise, measurements) {
  ...
  // Only consider numeric measurements for OEE calculation
  const numericMeasurements = measurements.filter(m => m.valueType === 'numeric');
...
async function calculateOEEv2(enterprise, site = null) {
  ...
  // Try Tier 0 first (data_type tag) - works for new data
  const tier0Result = await calculateTier0(enterprise, site);
  if (tier0Result.oee !== null) {
    return tier0Result;
  }

  // Fall back to discovery-based tiers for old data
...
/**
 * Tier 0: Query by data_type tag (preferred for new data)
 * Uses the topic classifier's data_type tagging for reliable OEE discovery
 */
async function calculateTier0(enterprise, site) {
  const fluxQuery = `
    from(bucket: "${CONFIG.influxdb.bucket}")
      |> range(start: -24h)
      |> filter(fn: (r) => r._field == "value")
      |> filter(fn: (r) => r.data_type == "oee")
  ...
  const components = { availability: null, performance: null, quality: null, oee: null };
  let dataPoints = 0;
  ...
  await new Promise((resolve) => {
    queryApi.queryRows(fluxQuery, {
      next(row, tableMeta) {
        ...
        if (o._value !== undefined && o._measurement) {
          const name = o._measurement.toLowerCase();
          let val = o._value;
          // Normalize decimal to percentage
          if (val > 0 && val <= 1.5) val = val * 100;
          val = Math.min(100, Math.max(0, val));
          
          if (name.includes('availability')) components.availability = val;
          else if (name.includes('performance')) components.performance = val;
          else if (name.includes('quality')) components.quality = val;
          else if (name.includes('oee') && !name.includes('_')) components.oee = val;
          dataPoints++;
        }
      },
      error(error) {
        console.error(`Tier 0 OEE query error for ${enterprise}:`, error.message);
        resolve();
      },
      complete() { resolve(); }
    });
  ...
  // Use pre-computed OEE if available, otherwise calculate from components
  let oeeValue = components.oee;
  if (oeeValue === null && components.availability && components.performance && components.quality) {
    oeeValue = (components.availability / 100) * (components.performance / 100) * (components.quality / 100) * 100;
  }

  if (oeeValue === null) {
    return createOEEResult(enterprise, site, null, null, 0, 'No data_type tagged OEE data');
  }

  return createOEEResult(enterprise, site, oeeValue, {
    availability: components.availability ? parseFloat(components.availability.toFixed(1)) : null,
    performance: components.performance ? parseFloat(components.performance.toFixed(1)) : null,
    quality: components.quality ? parseFloat(components.quality.toFixed(1)) : null
  }, 0, 'Using data_type tagged OEE metrics', { dataPoints });
}
