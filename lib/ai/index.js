...
${filterRulesSection}`;
  ...
  try {
    // Call anomaly agent instead of Bedrock directly
    const agentUrl = CONFIG.agents?.anomaly || process.env.AGENT_ANOMALY_URL || 'http://localhost:8082';
    const response = await fetch(`${agentUrl}/invocations`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ prompt, session_id: `anomaly_${Date.now()}` })
    });
    ...
    // Collect streaming response and parse SSE format
