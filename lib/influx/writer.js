...
const { getInfluxTags, extractPayloadValue } = require('../mqtt/topic-classifier');

// Track type conflicts for debugging
const typeConflicts = new Map();

/**
 * Parse MQTT topic into InfluxDB measurement and tags
 * Uses topic classifier for pattern-based tag extraction
...
function parseTopicToInflux(topic, payload) {
  ...
  const tags = getInfluxTags(topic);

  // Extract value from JSON if present
  const { value: extracted } = extractPayloadValue(payload);
  
  // Skip if no value (explicitly null in payload)
  if (extracted === null) return null;
  
  const value = parseFloat(extracted);
  const isNumeric = !isNaN(value);
  const valueType = isNumeric ? 'float' : 'string';

  // Measurement name from field (for classified topics) or last 2 topic parts
  const measurement = tags.field 
    ? tags.field.replace('/', '_') 
    : parts.slice(-2).join('_').replace(/[^a-zA-Z0-9_]/g, '_');

  // Track type conflicts
  const existing = typeConflicts.get(measurement);
  if (existing && existing.type !== valueType) {
    console.warn(`[TYPE CONFLICT] ${measurement}: was ${existing.type} (${existing.sample}), now ${valueType} (${String(extracted).substring(0, 50)})`);
    console.warn(`  topic: ${topic}`);
    try {
      const parsed = JSON.parse(payload);
      console.warn(`  payload keys: ${Object.keys(parsed).join(', ')}`);
      console.warn(`  value field: ${JSON.stringify(parsed.value)}`);
      console.warn(`  _name field: ${parsed._name}`);
    } catch (e) {
      console.warn(`  raw payload: ${String(payload).substring(0, 100)}`);
    }
  }
  typeConflicts.set(measurement, { type: valueType, sample: String(extracted).substring(0, 50), topic });

  const point = new Point(measurement)
    .tag('enterprise', tags.enterprise)
    .tag('site', tags.site)
    .tag('area', tags.area)
    .tag('machine', tags.machine)
    .tag('device', tags.device)
    .tag('data_type', tags.data_type)
    .tag('full_topic', topic);

  if (isNumeric) {
    point.floatField('value', value);
  } else {
    point.stringField('value', String(extracted).substring(0, 500));
  }
