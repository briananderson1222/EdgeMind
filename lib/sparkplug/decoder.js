...
function extractMetricValue(metric) {
  // Handle new format: { value: X, type: "Float" }
  if (metric.value !== undefined && metric.type) {
    const val = metric.value;
    const type = metric.type.toLowerCase();
    
    // Skip Template types (nested structures)
    if (type === 'template') {
      return { value: null, type: 'template' };
    }
    
    // Skip null-like string values
    if (type === 'string' && (val === '<nil>' || val === null)) {
  ...
  // Legacy format: check all possible value fields
...
function extractMetrics(topic, decodedPayload) {
  ...
  for (const metric of decodedPayload.metrics) {
    ...
    // For DDATA messages where groupId is a namespace (like 'jpi'), use edgeNodeId as enterprise
    const isNamespaceGroupId = /^(jpi|spBv1\.0|infrastructure|phoenixcontact)$/i.test(topicParts.groupId);
    const enterprise = isNamespaceGroupId ? topicParts.edgeNodeId : topicParts.groupId;
    ...
    metrics.push({
      ...
      tags: {
        ...
        messageType: topicParts.messageType
      }
    });
  }
